...
void NavigationQmlFramesTest::navigationCalendar_mustMountContentSurfaceInline()
{
    ...
    QVERIFY(contentViewLayout.contains(QStringLiteral("Layout.fillHeight: true")));
    QVERIFY(contentViewLayout.contains(QStringLiteral("Layout.fillWidth: true")));
    ...
    QVERIFY(calendarTodayControl.contains(QStringLiteral("readonly property string figmaNodeId: \"227:8807\"")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("readonly property int controlButtonExtent: 20")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("readonly property int todayButtonWidth: 50")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("spacing: LV.Theme.gap2")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("text: calendarTodayControl.todayText")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("font.pixelSize: 12")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("horizontalPadding: LV.Theme.gap8")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("verticalPadding: LV.Theme.gap4")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("height: calendarTodayControl.controlButtonExtent")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("width: calendarTodayControl.todayButtonWidth")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("iconName: \"generalchevronLeft\"")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("rotation: 180")));
    QVERIFY(calendarTodayControl.contains(QStringLiteral("tone: LV.AbstractButton.Default")));
    ...
    QVERIFY(dayCalendarPage.contains(QStringLiteral("color: \"transparent\"")));
    QVERIFY(!dayCalendarPage.contains(QStringLiteral("color: LV.Theme.panelBackground07")));
    QVERIFY(dayCalendarPage.contains(QStringLiteral("CalendarTodayControl {")));
    QVERIFY(dayCalendarPage.contains(QStringLiteral("onTodayRequested: dayCalendarPage.jumpToToday()")));
    QVERIFY(dayCalendarPage.contains(QStringLiteral("onPreviousRequested: {")));
    QVERIFY(dayCalendarPage.contains(QStringLiteral("onNextRequested: {")));
    ...
    QVERIFY(dayCalendarPage.contains(QStringLiteral("function entryCardLabel(entryModel)")));
    QVERIFY(dayCalendarPage.contains(QStringLiteral("readonly property real slotHeight: Math.max(")));
    ...
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int eventBackgroundColored: 1")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int monthHeaderHeight: 54")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int weekdayHeaderHeight: 39")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int headerHorizontalPadding: 8")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int weekdayCellHorizontalPadding: 12")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int bodyLabelPixelSize: 12")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property string figmaNodeId: \"228:9666\"")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int visibleWeekRowCount")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("function buildEntryCellModels(dayEntries)")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("GridLayout {")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("id: monthHeaderBand")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("id: weekdayHeaderBand")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("CalendarTodayControl {")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("onTodayRequested: monthCalendarPage.jumpToCurrentMonth()")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("monthCalendarPage.requestViewHook(\"previous-month\")")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("monthCalendarPage.requestViewHook(\"next-month\")")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("monthCalendarPage.requestViewHook(\"current-month\")")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("monthCalendarPage.calendarVm.focusToday")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("height: monthCalendarPage.weekdayHeaderHeight")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("Layout.minimumHeight: monthCalendarPage.weekdayHeaderHeight")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("anchors.leftMargin: monthCalendarPage.weekdayCellHorizontalPadding")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("font.pixelSize: monthCalendarPage.bodyLabelPixelSize")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("font.weight: Font.Medium")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("anchors.top: weekdayHeaderBand.bottom")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("anchors.bottom: parent.bottom")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("id: monthDayGrid")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("columns: 7")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("rowSpacing: 0")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("columnSpacing: 0")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int dayCellWidth: Math.max(1, Math.floor(monthDayGrid.width / 7))")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property int dayCellHeight: Math.max(")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("model: monthCalendarPage.visibleDayModels")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("color: \"transparent\"")));
    QVERIFY(!monthCalendarPage.contains(QStringLiteral("color: LV.Theme.panelBackground07")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("MonthCalendarDayCell {")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property var dayEntries: monthCalendarPage.entriesForDate(dayCell.dayModel)")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("readonly property var dayEntryCells: monthCalendarPage.buildEntryCellModels(dayCell.dayEntries)")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("Layout.fillHeight: true")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("Layout.preferredWidth: monthDayGrid.dayCellWidth")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("Layout.preferredHeight: monthDayGrid.dayCellHeight")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("disable: !dayCell.isCurrentMonth")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("entryCells: dayCell.dayEntryCells")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("maxVisibleEntries: monthCalendarPage.maxVisibleEntriesPerCell")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("selected: dayCell.isSelectedDate")));
    QVERIFY(monthCalendarPage.contains(QStringLiteral("today: dayCell.isToday")));
    QVERIFY(!monthCalendarPage.contains(QStringLiteral("Flickable {")));
    QVERIFY(!monthCalendarPage.contains(QStringLiteral("ListView {")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("property bool disable: false")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("property bool selected: false")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("property bool today: false")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("property var entryCells: []")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("readonly property int dayLabelHeight: 12")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("opacity: monthCalendarDayCell.disable ? 0.5 : 1.0")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("font.pixelSize: 12")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("font.weight: Font.Medium")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("CalendarEventCell {")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("backgroundType: eventCell.eventCellModel && eventCell.eventCellModel.backgroundType !== undefined")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("coloredBackgroundColor: eventCell.eventCellModel && eventCell.eventCellModel.backgroundColor !== undefined")));
    QVERIFY(monthCalendarDayCell.contains(QStringLiteral("text: \"+\" + String(monthCalendarDayCell.entryCells.length - monthCalendarDayCell.clippedEntryCount)")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("property string label: \"\"")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("readonly property int backgroundDefault: 0")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("readonly property int backgroundColored: 1")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("readonly property string figmaNodeId: \"227:9429\"")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("property int backgroundType: calendarEventCell.backgroundDefault")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("property color defaultBackgroundColor: LV.Theme.panelBackground08")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("property color coloredBackgroundColor: LV.Theme.primary")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("property int cornerRadius: 4")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("color: calendarEventCell.backgroundType === calendarEventCell.backgroundColored")));
    QVERIFY(calendarEventCell.contains(QStringLiteral(": calendarEventCell.defaultBackgroundColor")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("radius: calendarEventCell.cornerRadius")));
    QVERIFY(calendarEventCell.contains(QStringLiteral("font.pixelSize: calendarEventCell.labelPixelSize")));
    ...
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property bool mobileYearListMode: LV.Theme.mobileTarget")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("function jumpToCurrentYear()")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("yearCalendarPage.calendarVm.focusToday")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int desktopYearGridColumnCount: 4")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int desktopYearGridRowCount: 3")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int mobileYearGridColumnCount: 1")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int yearGridColumnCount: yearCalendarPage.mobileYearListMode ? yearCalendarPage.mobileYearGridColumnCount : yearCalendarPage.desktopYearGridColumnCount")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property real desktopResponsiveScale: yearCalendarPage.mobileYearListMode")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int yearGridSpacing: Math.max(")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int monthCardPadding: Math.max(")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int monthSectionSpacing: Math.max(")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int monthWeekdayPixelSize: 12")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int monthDayPixelSize: 12")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property color monthTitleColor: LV.Theme.accent")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int monthCellWidthFloor: Math.max")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("flickableDirection: Flickable.VerticalFlick")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("columns: yearCalendarPage.yearGridColumnCount")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("spacing: yearCalendarPage.yearGridSpacing")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("anchors.margins: yearCalendarPage.monthCardPadding")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("id: calendarToolbar")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("visible: true")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("CalendarTodayControl {")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("onTodayRequested: yearCalendarPage.jumpToCurrentYear()")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("visible: yearCalendarPage.mobileYearListMode")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("height: yearCalendarPage.monthWeekdayPixelSize + yearCalendarPage.monthSectionSpacing")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("font.pixelSize: yearCalendarPage.monthWeekdayPixelSize")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("font.pixelSize: yearCalendarPage.monthDayPixelSize")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("font.weight: Font.Medium")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("readonly property int dayCellHeight: Math.max(")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("color: \"transparent\"")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("color: yearCalendarPage.monthTitleColor")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("color: yearCalendarPage.todayBadgeColor")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("width: monthCard.dayCellWidth")));
    QVERIFY(yearCalendarPage.contains(QStringLiteral("color: \"transparent\"")));
    QVERIFY(!yearCalendarPage.contains(QStringLiteral("color: LV.Theme.panelBackground07")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("color: \"transparent\"")));
    QVERIFY(!weekCalendarPage.contains(QStringLiteral("color: LV.Theme.panelBackground07")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("CalendarTodayControl {")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("CalendarEventCell {")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("function slotBackgroundType(entries)")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("defaultBackgroundColor: LV.Theme.panelBackground11")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("label: weekCalendarPage.slotSummary(dayHourCell.slotEntries)")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("onTodayRequested: weekCalendarPage.jumpToCurrentWeek()")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("ListView {")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("orientation: ListView.Horizontal")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("snapMode: ListView.SnapOneItem")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property bool horizontalDayScrollEnabled: LV.Theme.mobileTarget")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property int mobileMinimumDayColumnWidth: LV.Theme.gap24 * 3")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property int fittedDayColumnWidth")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property int minimumDayColumnWidth: weekCalendarPage.horizontalDayScrollEnabled")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property bool dayColumnScrollEnabled: weekCalendarPage.horizontalDayScrollEnabled")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("readonly property real hourRowHeight: Math.max(")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("height: weekPage.hourRowHeight")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("flickableDirection: Flickable.HorizontalFlick")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("interactive: weekPage.dayColumnScrollEnabled")));
    QVERIFY(!weekCalendarPage.contains(QStringLiteral("? Flickable.HorizontalAndVerticalFlick")));
    QVERIFY(!weekCalendarPage.contains(QStringLiteral(": Flickable.VerticalFlick")));
    QVERIFY(!weekCalendarPage.contains(QStringLiteral("text: weekCalendarPage.horizontalDayScrollEnabled ? \"Swipe dates\" : \"Swipe week\"")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("function ensureLazyWindow(index)")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("function prependWeeks(count)")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("function appendWeeks(count)")));
    QVERIFY(weekCalendarPage.contains(QStringLiteral("onCurrentIndexChanged: {")));
