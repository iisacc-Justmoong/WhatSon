...
class CliOnboardingTests(unittest.TestCase):
    ...
    def test_app_supports_onboarding_only_launch_mode(self) -> None:
        ...
        launch_support_text = (
                REPO_ROOT / "src/app/runtime/bootstrap/WhatSonAppLaunchSupport.cpp"
        ).read_text(encoding="utf-8")
        startup_resolver_header_text = (
                REPO_ROOT / "src/app/runtime/startup/WhatSonStartupHubResolver.hpp"
        ).read_text(encoding="utf-8")
        startup_resolver_impl_text = (
                REPO_ROOT / "src/app/runtime/startup/WhatSonStartupHubResolver.cpp"
        ).read_text(encoding="utf-8")

        self.assertIn("WhatSon::Runtime::Bootstrap::parseLaunchOptions(app);", app_main_text)
        self.assertIn('#include "backend/runtime/appbootstrap.h"', app_main_text)
        self.assertIn('QStringList{QStringLiteral("onboarding-only")}', launch_support_text)
        self.assertIn("QCommandLineParser parser;", launch_support_text)
        ...
        self.assertIn('#include "runtime/startup/WhatSonStartupHubResolver.hpp"', app_main_text)
        self.assertIn("const QString blueprintFallbackHubPath = WhatSon::Runtime::Bootstrap::resolveBlueprintHubPath();", app_main_text)
        self.assertIn("const WhatSon::Runtime::Startup::StartupHubSelection startupHubSelection =", app_main_text)
        self.assertIn("WhatSon::Runtime::Startup::resolveStartupHubSelection(selectedHubStore, blueprintFallbackHubPath);", app_main_text)
        self.assertIn("if (startupHubSelection.mounted)", app_main_text)
        self.assertIn("const bool showDesktopStartupOnboarding = !startupHubSelection.mounted && !useEmbeddedStartupOnboarding;", app_main_text)
        self.assertIn("onboardingRouteBootstrapController.configure(useEmbeddedStartupOnboarding, startupHubSelection.mounted);", app_main_text)
        ...
        self.assertIn("onboardingHubController.syncCurrentHubSelection(startupHubSelection.hubPath);", app_main_text)
        self.assertIn("SelectedHubStore selectedHubStore;", app_main_text)
        self.assertIn("selectedHubStore.setSelectedHubSelection(", app_main_text)
        self.assertIn("onboardingHubController.currentHubAccessBookmark()", app_main_text)
        self.assertIn("QString resolveStartupHubMountPath(", startup_resolver_header_text)
        self.assertIn("StartupHubSelection resolveStartupHubSelection(", startup_resolver_header_text)
        self.assertIn("const QString startupHubSelectionPath = selectedHubStore.startupHubPath(blueprintFallbackHubPath);", startup_resolver_impl_text)
        self.assertIn("const QByteArray startupHubAccessBookmark = selectedHubStore.selectedHubAccessBookmark();", startup_resolver_impl_text)
        self.assertIn("const QString normalizedHubPath = hubPath.trimmed().isEmpty()", startup_resolver_impl_text)
        self.assertIn("WhatSon::Android::Storage::isSupportedUri(normalizedHubPath)", startup_resolver_impl_text)
        self.assertIn("WhatSon::Android::Storage::isMountedHubPath(normalizedHubPath)", startup_resolver_impl_text)
        self.assertIn("WhatSon::Android::Storage::mountedHubSourceUri(normalizedHubPath);", startup_resolver_impl_text)
        self.assertIn("WhatSon::Android::Storage::mountHub(normalizedHubPath, &mountedHubPath, errorMessage)", startup_resolver_impl_text)
        self.assertIn("WhatSon::Android::Storage::mountHub(sourceUri, &remountedHubPath, errorMessage)", startup_resolver_impl_text)
        self.assertIn("WhatSon::Apple::SecurityScopedResourceAccess::restoreAccessFromBookmarkData(", startup_resolver_impl_text)
        self.assertIn("WhatSon::Apple::SecurityScopedResourceAccess::ensureAccessForPath(normalizedHubPath, &iosAccessError)", startup_resolver_impl_text)
        self.assertIn("const QString parentDirectoryPath = QFileInfo(normalizedHubPath).absolutePath();", startup_resolver_impl_text)
        self.assertIn("iOS denied access to the stored WhatSon Hub path during startup.", startup_resolver_impl_text)
        self.assertIn("Failed to resolve %1 startup WhatSon Hub mount '%2': %3", startup_resolver_impl_text)
        self.assertIn('QStringLiteral("persisted")', startup_resolver_impl_text)
        self.assertIn('QStringLiteral("blueprint fallback")', startup_resolver_impl_text)
        self.assertIn("WhatSon::HubPath::normalizePath(startupHubSelectionPath)", startup_resolver_impl_text)
        self.assertIn("WhatSon::HubPath::normalizePath(blueprintFallbackHubPath)", startup_resolver_impl_text)
        ...
        self.assertIn("readonly property string expectedEmbeddedRoutePath: {", main_qml_text)
        self.assertIn("readonly property bool embeddedRouteHostReady: {", main_qml_text)
        self.assertIn("readonly property bool embeddedRouteRecoveryNeeded: {", main_qml_text)
        self.assertIn('return "Restoring WhatSon startup route...";', main_qml_text)
        self.assertIn("readonly property int startupRouteRecoveryLimit: 3", main_qml_text)
        self.assertIn("property int startupRouteRecoveryAttempts: 0", main_qml_text)
        ...
        self.assertIn('readonly property bool useIosSystemWindowingPolicy: applicationWindow.platform === "ios"', main_qml_text)
        self.assertIn("delegateMobileInsetsToSystem: applicationWindow.useIosSystemWindowingPolicy", main_qml_text)
        self.assertIn("delegateMobileWindowingToSystem: applicationWindow.useIosSystemWindowingPolicy", main_qml_text)
        self.assertIn("forceFullWindowAreaOnMobile: applicationWindow.isMobilePlatform && !applicationWindow.useIosSystemWindowingPolicy", main_qml_text)
        self.assertIn("usePlatformSafeMargin: false", main_qml_text)
        self.assertIn("forcedDeviceTierPreset: applicationWindow.useIosSystemWindowingPolicy ? LV.RenderQuality.LowTier : -1", main_qml_text)
        self.assertIn("function applyRequestedRoute(targetPath, routeSource, forceReload)", main_qml_text)
        self.assertIn("applicationWindow.activePageRouter.setRoot(targetPath);", main_qml_text)
        self.assertIn("function recoverEmbeddedRouteHost(source)", main_qml_text)
        self.assertIn("function syncEmbeddedRouteWatchdog(source)", main_qml_text)
        self.assertIn('applicationWindow.applyRequestedRoute(', main_qml_text)
        self.assertIn('"startupWatchdog:" + source,', main_qml_text)
        self.assertIn("forceReload);", main_qml_text)
        ...
        self.assertIn('import "view/mobile/pages" as MobilePageView', main_qml_text)
        self.assertIn("MobilePageView.MobileHierarchyPage {", main_qml_text)
        ...
        self.assertIn("applicationWindow.syncEmbeddedRouteWatchdog(\"completed\");", main_qml_text)
        self.assertIn("target: applicationWindow.activePageRouter", main_qml_text)
        self.assertIn("function onCurrentPageItemChanged()", main_qml_text)
        self.assertIn("function onCurrentPathChanged()", main_qml_text)
        self.assertIn("id: embeddedRouteStartupFallback", main_qml_text)
        self.assertIn("applicationWindow.embeddedRouteRecoveryNeeded", main_qml_text)

        main_window_interaction_text = (
                REPO_ROOT / "src/app/qml/MainWindowInteractionController.qml"
        ).read_text(encoding="utf-8")
        self.assertIn("readonly property bool resizeRenderGuardSupported: interactionController.hostWindow", main_window_interaction_text)
        self.assertIn('? interactionController.hostWindow.isDesktopPlatform', main_window_interaction_text)
        self.assertIn('action=" + guardPolicy + " dynamicResolutionEnabled="', main_window_interaction_text)
        self.assertIn('|| !interactionController.resizeRenderGuardSupported', main_window_interaction_text)
        ...
        self.assertIn("bool ensureAccessForPath(const QString& localPath, QString* errorMessage);",
                      ios_access_header_text)
        self.assertIn("QByteArray bookmarkDataForUrl(const QUrl& url, bool parentDirectoryScope, QString* errorMessage);",
                      ios_access_header_text)
        self.assertIn("bool restoreAccessFromBookmarkData(", ios_access_header_text)
        self.assertIn("[nsUrl startAccessingSecurityScopedResource]", ios_access_impl_text)
        self.assertIn("bookmarkDataWithOptions:0", ios_access_impl_text)
        self.assertIn("URLByResolvingBookmarkData:bookmarkNsData", ios_access_impl_text)
        self.assertIn("NSURLBookmarkResolutionWithoutMounting", ios_access_impl_text)
        ...
        self.assertIn("Failed to create a persistent bookmark for the selected iOS WhatSon Hub.", ios_access_impl_text)
        self.assertIn("Failed to restore the stored iOS WhatSon Hub bookmark.", ios_access_impl_text)
        ...
        self.assertIn('workspace/selectedHubBookmark', selected_hub_store_text)
        self.assertIn("QByteArray SelectedHubStore::selectedHubAccessBookmark()", selected_hub_store_text)
        self.assertIn("void SelectedHubStore::setSelectedHubSelection(const QString& hubPath, const QByteArray& accessBookmark)", selected_hub_store_text)
