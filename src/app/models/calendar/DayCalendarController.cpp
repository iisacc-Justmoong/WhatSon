...
DayCalendarController::DayCalendarController(QObject* parent)
    : QObject(parent)
...
QString DayCalendarController::displayedDateIso() const
{
    return m_displayedDateIso;
}

QString DayCalendarController::dayLabel() const
{
    return m_dayLabel;
}

QVariantList DayCalendarController::dayEntries() const
{
    return m_dayEntries;
}

QVariantList DayCalendarController::timeSlots() const
{
    return m_timeSlots;
}

void DayCalendarController::setCalendarBoardStore(ICalendarBoardStore* calendarBoardStore)
{
    if (calendarBoardStore != nullptr
        && !WhatSon::Policy::verifyMutableDependencyAllowed(
            ...
            QStringLiteral("DayCalendarController::setCalendarBoardStore")))
    {
        return;
    }
...
void DayCalendarController::setDisplayedDateIso(const QString& dateIso)
{
    QDate parsedDate;
...
void DayCalendarController::shiftDay(int deltaDays)
{
    if (deltaDays == 0)
    {
        return;
...
void DayCalendarController::requestDayView(const QString& reason)
{
...
bool DayCalendarController::addEvent(
...
bool DayCalendarController::addTask(
    const QString& dateIso,
    const QString& timeText,
    const QString& title,
    const QString& detail)
...
QVariantList DayCalendarController::entriesForDate(const QString& dateIso) const
{
    if (!m_calendarBoardStore)
    {
        return {};
...
bool DayCalendarController::removeEntry(const QString& entryId)
{
    if (!m_calendarBoardStore)
    {
        return false;
...
bool DayCalendarController::setTaskCompleted(const QString& entryId, bool completed)
{
    if (!m_calendarBoardStore)
    {
        return false;
...
void DayCalendarController::rebuildDayModel()
{
    QDate dayDate;
...
bool DayCalendarController::parseIsoDate(const QString& dateIso, QDate* outDate)
{
    if (!outDate)
    {
        return false;
...
QString DayCalendarController::formatDayLabel(const QDate& date, const QLocale& locale)
{
