...
WeekCalendarController::WeekCalendarController(QObject* parent)
    : QObject(parent)
{
...
QString WeekCalendarController::displayedWeekStartIso() const
{
    return m_displayedWeekStartIso;
}

QString WeekCalendarController::weekLabel() const
{
    return m_weekLabel;
}

QStringList WeekCalendarController::weekdayLabels() const
{
    return m_weekdayLabels;
}

QVariantList WeekCalendarController::dayModels() const
{
    return m_dayModels;
}

QVariantList WeekCalendarController::timelineDayModels() const
{
    return m_timelineDayModels;
}

void WeekCalendarController::setCalendarBoardStore(ICalendarBoardStore* calendarBoardStore)
{
    if (calendarBoardStore != nullptr
        && !WhatSon::Policy::verifyMutableDependencyAllowed(
            ...
            QStringLiteral("WeekCalendarController::setCalendarBoardStore")))
    {
        return;
    }
...
void WeekCalendarController::setDisplayedWeekStartIso(const QString& dateIso)
{
    QDate parsedDate;
...
void WeekCalendarController::shiftWeek(int deltaWeeks)
{
    if (deltaWeeks == 0)
    {
        return;
...
void WeekCalendarController::requestWeekView(const QString& reason)
{
...
void WeekCalendarController::initializeTimelineWindow(const QString& anchorDateIso, int radius)
{
    QDate anchorDate;
...
int WeekCalendarController::prependTimelineDates(int count)
{
...
int WeekCalendarController::appendTimelineDates(int count)
{
    if (count <= 0 || m_timelineDayModels.isEmpty())
    {
        return 0;
...
QVariantMap WeekCalendarController::trimTimelineWindow(int leadingIndex, int maxWindowSize, int chunkSize)
{
    QVariantMap result{
...
bool WeekCalendarController::addEvent(
...
bool WeekCalendarController::addTask(
    const QString& dateIso,
    const QString& timeText,
    const QString& title,
    const QString& detail)
...
QVariantList WeekCalendarController::entriesForDate(const QString& dateIso) const
{
    if (!m_calendarBoardStore)
    {
        return {};
...
bool WeekCalendarController::removeEntry(const QString& entryId)
{
    if (!m_calendarBoardStore)
    {
        return false;
...
bool WeekCalendarController::setTaskCompleted(const QString& entryId, bool completed)
{
    if (!m_calendarBoardStore)
    {
        return false;
...
QVariantMap WeekCalendarController::buildTimelineDayModel(const QDate& date, const QLocale& locale) const
{
    if (!date.isValid())
    {
        return {};
...
void WeekCalendarController::refreshTimelineDayModels()
{
...
void WeekCalendarController::rebuildWeekModel()
{
    const QLocale locale = QLocale::system();
...
bool WeekCalendarController::parseIsoDate(const QString& dateIso, QDate* outDate)
{
    if (!outDate)
    {
        return false;
...
QDate WeekCalendarController::startOfWeek(const QDate& date, const QLocale& locale)
{
    if (!date.isValid())
    {
        return {};
...
QString WeekCalendarController::formatWeekLabel(const QDate& weekStartDate, const QLocale& locale)
{
