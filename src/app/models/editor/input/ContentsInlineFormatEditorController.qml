...
Item {
    ...
    function macOsOptionWordNavigationTarget(direction, cursorPosition) {
        const textValue = controller.currentPlainText();
        const textLength = textValue.length;
        const cursor = controller.clampLogicalPosition(cursorPosition, textLength);
        ...
        if (normalizedDirection === "right") {
            let index = cursor;
            if (index < textLength && controller.macOsTextUnitIsWord(textValue.charAt(index))) {
                while (index < textLength && controller.macOsTextUnitIsWord(textValue.charAt(index)))
                    ++index;
                return index;
            }
            while (index < textLength && !controller.macOsTextUnitIsWord(textValue.charAt(index)))
        ...
        if (normalizedDirection === "left") {
            let index = cursor - 1;
            while (index > 0 && !controller.macOsTextUnitIsWord(textValue.charAt(index)))
                --index;
            while (index > 0 && controller.macOsTextUnitIsWord(textValue.charAt(index - 1)))
                --index;
    ...
    function handleMacOsOptionWordNavigation(event) {
        if (Qt.platform.os !== "osx" || !event || !controller.textInput || controller.nativeCompositionActive())
            return false;
        if (!controller.control || !controller.control.selectByKeyboard || controller.control.blockExternalDropMutation)
            return false;
        ...
        const target = controller.macOsOptionWordNavigationTarget(
                    event.key === Qt.Key_Left ? "left" : "right",
                    controller.textInput.cursorPosition);
        if (shiftPressed) {
            if (controller.textInput.moveCursorSelection !== undefined) {
                controller.textInput.moveCursorSelection(target, TextEdit.SelectCharacters);
            } else if (controller.textInput.select !== undefined) {
                const anchor = Math.max(0, Math.floor(Number(controller.textInput.cursorPosition) || 0));
                controller.textInput.select(Math.min(anchor, target), Math.max(anchor, target));
                controller.textInput.cursorPosition = target;
            } else {
                controller.textInput.cursorPosition = target;
            }
        } else {
