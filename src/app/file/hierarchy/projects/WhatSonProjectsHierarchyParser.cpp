    ...
    QString* errorMessage) const
{
    ...
    if (parseError.error == QJsonParseError::NoError && !document.isNull())
    {
        if (document.isArray())
        {
            ...
            outStore->setProjectNames(parsedValues);
            return true;
        }
        ...
        if (document.isObject())
        {
            ...
            const QJsonValue listValue = object.value(QStringLiteral("projects"));
            if (listValue.isArray())
            {
                outStore->setProjectNames(parseArrayValues(listValue.toArray()));
                return true;
            }
            if (listValue.isString())
            {
                outStore->setProjectNames(QStringList{listValue.toString().trimmed()});
                return true;
            }
        }
    }
