...
QtObject {
    ...
    function normalizedTrimmedTextValue(value) {
        return controller.normalizedTextValue(value).trim();
    }

    function selectedNoteDirectoryPath() {
        return controller.normalizedTrimmedTextValue(controller.contentsView.selectedNoteDirectoryPath);
    }

    function editorSessionBoundToSelectedNote() {
        if (!controller.editorSession || !controller.contentsView)
            return false;
        const selectedNoteId = controller.normalizedTrimmedTextValue(controller.contentsView.selectedNoteId);
        const editorBoundNoteId = controller.normalizedTrimmedTextValue(controller.editorSession.editorBoundNoteId);
        if (selectedNoteId.length === 0 || editorBoundNoteId !== selectedNoteId)
            return false;
        const selectedDirectoryPath = controller.selectedNoteDirectoryPath();
        if (selectedDirectoryPath.length === 0)
            return true;
        const editorDirectoryPath = controller.normalizedTrimmedTextValue(controller.editorSession.editorBoundNoteDirectoryPath);
        return editorDirectoryPath === selectedDirectoryPath;
    }

    function ensureEditorSessionBoundForSourceMutation(currentSourceText) {
        if (controller.editorSessionBoundToSelectedNote())
            return true;
        if (!controller.editorSession
        ...
        if (selectedNoteId.length === 0)
            return false;
        controller.editorSession.requestSyncEditorTextFromSelection(
                    selectedNoteId,
                    controller.normalizedTextValue(currentSourceText),
                    selectedNoteId,
                    controller.selectedNoteDirectoryPath());
        return controller.editorSessionBoundToSelectedNote();
    }

    function currentDocumentSourceText() {
        if (controller.contentsView
                && controller.contentsView.documentPresentationSourceText !== undefined
                && controller.contentsView.documentPresentationSourceText !== null) {
            return String(controller.contentsView.documentPresentationSourceText);
        }
        if (controller.contentsView
                && controller.contentsView.editorText !== undefined
                && controller.contentsView.editorText !== null) {
            return String(controller.contentsView.editorText);
        }
        return "";
    }

    function focusRequestRequiresImmediatePersistence(focusRequest) {
        return !!(focusRequest
                  && typeof focusRequest === "object"
                  && focusRequest.immediatePersistence !== undefined
                  && focusRequest.immediatePersistence);
    }

    function persistRawEditorTextImmediately(nextSourceText) {
        if (!controller.editorSession)
            return false;
        if (controller.editorSession.persistEditorTextImmediatelyWithText !== undefined) {
            return !!controller.editorSession.persistEditorTextImmediatelyWithText(nextSourceText);
        }
        if (controller.editorSession.persistEditorTextImmediately !== undefined) {
    ...
    function applyDocumentSourceMutation(nextSourceText, focusRequest) {
        const currentSourceText = controller.currentDocumentSourceText();
        ...
        if (normalizedNextSourceText === currentSourceText)
            return false;
        if (!controller.ensureEditorSessionBoundForSourceMutation(currentSourceText))
            return false;
        ...
        const immediatePersistenceRequired = controller.focusRequestRequiresImmediatePersistence(focusRequest);
        if (immediatePersistenceRequired)
            controller.persistRawEditorTextImmediately(committedText);
        if (focusRequest
                && controller.structuredDocumentFlow
    ...
    function persistEditorTextImmediately(nextText) {
        return controller.persistRawEditorTextImmediately(nextText);
    }
