...
PresetHierarchyModel::PresetHierarchyModel(QObject* parent)
    : QAbstractListModel(parent)
{
    WhatSon::Debug::traceSelf(this, QStringLiteral("hierarchy.preset.model"), QStringLiteral("ctor"));
}

int PresetHierarchyModel::rowCount(const QModelIndex& parent) const
{
    if (parent.isValid())
    {
        return 0;
...
QVariant PresetHierarchyModel::data(const QModelIndex& index, int role) const
{
    ...
    const PresetHierarchyItem& item = m_items.at(index.row());
    switch (role)
    {
    case LabelRole:
        return item.label;
...
QHash<int, QByteArray> PresetHierarchyModel::roleNames() const
{
    return {
        {LabelRole, "label"},
        {DepthRole, "depth"},
...
int PresetHierarchyModel::itemCount() const noexcept
{
    return m_items.size();
}

bool PresetHierarchyModel::strictValidation() const noexcept
{
    return m_strictValidation;
}

void PresetHierarchyModel::setStrictValidation(bool enabled)
{
...
int PresetHierarchyModel::correctionCount() const noexcept
{
    return m_correctionCount;
}

QString PresetHierarchyModel::lastValidationCode() const
{
    return m_lastValidationCode;
}

QString PresetHierarchyModel::lastValidationMessage() const
{
    return m_lastValidationMessage;
}

void PresetHierarchyModel::setItems(QVector<PresetHierarchyItem> items)
{
    const int previousCount = m_items.size();
    QVector<PresetHierarchyItem> sanitized;
    ...
    {
        PresetHierarchyItem item = std::move(items[index]);
        if (item.depth < 0)
        {
    ...
    WhatSon::Debug::traceSelf(this,
                              QStringLiteral("hierarchy.preset.model"),
...
const QVector<PresetHierarchyItem>& PresetHierarchyModel::items() const noexcept
{
    return m_items;
}

void PresetHierarchyModel::setValidationState(QString code, QString message)
{
    code = code.trimmed();
