...
namespace WhatSon::Hierarchy::EventSupport
{
    ...
    inline void applyChevronByDepth(QVector<EventHierarchyItem>* items)
    {
        if (items == nullptr)
        {
            return;
    ...
    inline int nextGeneratedFolderSequence(const QVector<EventHierarchyItem>& items)
    {
    ...
    inline bool renameHierarchyItem(QVector<EventHierarchyItem>* items, int index, const QString& displayName)
    {
    ...
    inline bool isBucketHeaderItem(const EventHierarchyItem& item)
    {
    ...
    inline int deleteHierarchySubtree(QVector<EventHierarchyItem>* items, int selectedIndex)
    {
    ...
    inline QStringList extractDomainLabelsFromItems(const QVector<EventHierarchyItem>& items)
    {
        ...
        for (int index = 0; index < items.size(); ++index)
        {
            const EventHierarchyItem& item = items.at(index);
