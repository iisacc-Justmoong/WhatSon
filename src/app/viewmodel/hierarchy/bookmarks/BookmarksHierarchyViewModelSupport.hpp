...
namespace WhatSon::Hierarchy::BookmarksSupport
{
    ...
    inline BookmarksHierarchyItem parseItemEntry(const QVariant& entry, int fallbackOrdinal,
    ...
    inline QVector<BookmarksHierarchyItem> parseDepthItems(
    ...
    inline QVariantList serializeDepthItems(const QVector<BookmarksHierarchyItem>& items)
    {
        QVariantList serialized;
        serialized.reserve(items.size());

        for (const BookmarksHierarchyItem& item : items)
        {
    ...
    inline QVector<BookmarksHierarchyItem> buildBucketItems(
    ...
    inline void applyChevronByDepth(QVector<BookmarksHierarchyItem>* items)
    {
        if (items == nullptr)
        {
            return;
    ...
    inline int nextGeneratedFolderSequence(const QVector<BookmarksHierarchyItem>& items)
    {
    ...
    inline bool renameHierarchyItem(QVector<BookmarksHierarchyItem>* items, int index, const QString& displayName)
    {
    ...
    inline bool isBucketHeaderItem(const BookmarksHierarchyItem& item)
    {
        return item.depth == 0 && item.accent;
    }

    inline int createHierarchyFolder(QVector<BookmarksHierarchyItem>* items, int selectedIndex, int* ioFolderSequence)
    {
    ...
    inline int deleteHierarchySubtree(QVector<BookmarksHierarchyItem>* items, int selectedIndex)
    {
    ...
    inline QStringList extractDomainLabelsFromItems(const QVector<BookmarksHierarchyItem>& items)
    {
        QStringList labels;
        ...
        for (int index = 0; index < items.size(); ++index)
        {
            const BookmarksHierarchyItem& item = items.at(index);
