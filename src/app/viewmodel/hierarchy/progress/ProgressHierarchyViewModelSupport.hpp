...
namespace WhatSon::Hierarchy::ProgressSupport
{
    ...
    {
        const QStringList sanitizedStates = sanitizeStringList(progressStates);

        QVector<ProgressHierarchyItem> items;
        items.reserve(sanitizedStates.size());

        for (const QString& stateLabel : sanitizedStates)
        {
            ProgressHierarchyItem item;
            item.depth = 0;
            item.accent = false;
    ...
    inline void applyChevronByDepth(QVector<ProgressHierarchyItem>* items)
    {
        if (items == nullptr)
        {
            return;
    ...
    inline int nextGeneratedFolderSequence(const QVector<ProgressHierarchyItem>& items)
    {
    ...
    inline bool renameHierarchyItem(QVector<ProgressHierarchyItem>* items, int index, const QString& displayName)
    {
    ...
    inline bool isBucketHeaderItem(const ProgressHierarchyItem& item)
    {
    ...
    inline int deleteHierarchySubtree(QVector<ProgressHierarchyItem>* items, int selectedIndex)
    {
    ...
    inline QStringList extractDomainLabelsFromItems(const QVector<ProgressHierarchyItem>& items)
    {
        ...
        for (int index = 0; index < items.size(); ++index)
        {
            const ProgressHierarchyItem& item = items.at(index);
