...
- Synchronizes the active note-list model plus hierarchy directory resolver into `NoteDetailPanelViewModel`.
- Detects when the active hierarchy index is `Resources` and only then forwards the active note-list model into
  `ResourceDetailPanelViewModel::setCurrentResourceListModel(...)`.
