...
- `ClipboardResourceImport.*`
  Maps file names and MIME types to the same resource taxonomy used by `.wsresources` packages.
- `InAppClipboard.*`
  Stores one in-app clipboard resource at a time, captures supported system clipboard payloads, and exposes the QML
  context object named `inAppClipboard`.
- `InAppClipboardResourceImport.cpp`
