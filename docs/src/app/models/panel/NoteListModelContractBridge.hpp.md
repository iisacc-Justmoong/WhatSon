...
- Accepts either:
  ...
  - a `QObject*` `hierarchyController` whose `hierarchyNoteListModel` / `noteListModel` contract identifies the active
...
- Exposes normalized read contracts:
  ...
  - `currentNoteEntry`
  - `currentNoteId`
  - `readCurrentNoteEntry()` for exporting the active note-backed row snapshot as a single QVariantMap payload,
    mirroring the resources hierarchy's `currentResourceEntry` pattern
...
The bridge now also exposes a `currentNoteEntry` contract so note-backed hierarchies can follow the same explicit
"current selection entry" shape that already works for resources via `currentResourceEntry`.
