...
It adapts a generic `QObject*` hierarchy controller input into a capability-aware surface that answers questions such as:
...
QML can pass object references easily, but it does not know about C++ interface inheritance. The bridge therefore accepts a raw `QObject*`, casts it to `IHierarchyController`, and then probes for capability interfaces at runtime.
