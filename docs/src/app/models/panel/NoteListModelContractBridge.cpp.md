...
- Resolves the effective note-list model from either:
  ...
  - the bound `hierarchyController`'s `hierarchyNoteListModel` / `noteListModel` contract.
...
- Forces resolved hierarchy/controller-owned QObject instances back to `QQmlEngine::CppOwnership`, so QML rebinding does
...
- immediate hierarchy-controller -> note-list-model rebinding without waiting for an extra event turn
- explicit note-list-model overrides must switch exported row snapshots immediately, including folder/tag metadata,
  even if the hierarchy-controller input is changing in the same transition window
