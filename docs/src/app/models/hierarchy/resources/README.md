...
- `ResourcesHierarchyController::syncModel()` publishes `depthItems()` into that shared model, and
  `setItemExpanded(...)` updates only the changed `expanded` row through `WhatSonHierarchyModel::setItemExpanded(...)`.
...
- 현재 규칙: 리소스 hierarchy도 표시용 item model은 공통 `WhatSonHierarchyModel`을 사용하고, domain struct는
  `ResourcesHierarchyModel.hpp`에만 남긴다. `syncModel()`은 `depthItems()`를 공용 모델에 publish하고,
  chevron 단일 변경은 `setItemExpanded(...)` row 갱신으로 처리한다.
