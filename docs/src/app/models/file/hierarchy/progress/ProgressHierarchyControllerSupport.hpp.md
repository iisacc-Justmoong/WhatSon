# `src/app/models/file/hierarchy/progress/ProgressHierarchyControllerSupport.hpp`
...
- Serialized depth items include the row label and structural metadata needed by the shared
  hierarchy QML layer, while the controller adds `progressValue` and `itemId` on top of that output.
