# `src/app/models/file/hierarchy/resources/ResourcesHierarchyControllerSupport.hpp`
...
These keys are the persistence anchor for `ResourcesHierarchyController` expansion-restore logic.
...
`ResourcesHierarchyController::setResourcePaths(...)` uses this comparator to skip no-op model resets
