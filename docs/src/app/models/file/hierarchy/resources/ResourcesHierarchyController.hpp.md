# `src/app/models/file/hierarchy/resources/ResourcesHierarchyController.hpp`

## Responsibility

This header declares the read-mostly resources hierarchy controller. It presents resources as a
...
- Implements `IHierarchyExpansionCapability` so open/closed state is owned by the controller.
...
- Exposes `requestControllerHook()` as a file-backed refresh hook that re-reads
