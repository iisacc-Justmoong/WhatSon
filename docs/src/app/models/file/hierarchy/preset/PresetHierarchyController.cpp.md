# `src/app/models/file/hierarchy/preset/PresetHierarchyController.cpp`
...
`requestControllerHook()` performs file-backed self-refresh when the preset source path is known.

- It reloads `Preset.wspreset` through `reloadFromPresetFilePath(...)`.
- On success, the controller re-applies the snapshot pipeline and emits `loadStateChanged` only when
  state actually changed.
- On read/parse failure, it keeps the current rows, records the failure in load-state, and still
  emits `controllerHookRequested()` so external hook listeners remain deterministic.
...
- When rows are available, a negative or invalid selected index is normalized to the first visible row so runtime
  refreshes do not leave the controller in an invisible "no selection" state.
