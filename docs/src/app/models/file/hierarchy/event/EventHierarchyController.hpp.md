# `src/app/models/file/hierarchy/event/EventHierarchyController.hpp`

## Responsibility

This header declares the dedicated event hierarchy controller. It presents the `Event.wsevent`
...
- Exposes `requestControllerHook()` as a file-backed hook entry point that can reload
  `Event.wsevent` from the persisted source path.
- Exposes `setItemExpanded(int, bool)` so fold state belongs to the controller instead of to a
