# `src/app/models/file/hierarchy/event/EventHierarchyController.cpp`
...
- If the list changed, the controller rebuilds the bucket rows, restores expansion by stable row key,
...
`requestControllerHook()` is no longer a signal-only bridge.
...
- Reload failures now surface through `loadStateChanged` with a concrete parser/read error while
  still emitting `controllerHookRequested()` for hook-chain compatibility.
