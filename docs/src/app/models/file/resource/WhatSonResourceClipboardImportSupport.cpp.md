...
- Platform image-like MIME names such as `public.png`, `public.tiff`, `com.apple.tiff`, or generic Qt image formats.
...
`test/cpp/suites/resources_import_controller_tests.cpp` verifies platform MIME payload extraction, generic Qt image MIME
payload extraction, macOS screenshot-style image names, and image-object payload extraction.
...
- `application/x-qt-image`나 `com.apple.tiff`처럼 `image/`로 시작하지 않는 플랫폼 이름도 이미지 payload 후보로
  검사한다.
