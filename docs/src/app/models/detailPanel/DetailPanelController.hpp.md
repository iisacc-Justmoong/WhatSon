...
## Responsibility
`DetailPanelController` owns the active detail-panel page state, toolbar selection state, the dedicated `fileStat`
...
`NoteDetailPanelController`, which inherits this contract unchanged.

## Owned Objects
- `DetailPropertiesController` for `properties`
- `DetailFileStatController` for `fileStat`
- `DetailContentSectionController` instances for:
  - `insert`
  - `fileHistory`
  - `layer`
  - `help`
- `DetailHierarchySelectionController` instances for:
...
The detail panel no longer binds QML selectors directly to the sidebar hierarchy controllers.
Instead, C++ injects those hierarchy controllers as read-only option sources into the owned selector-copy objects while a separate current-note context bridge resolves the active note id and note directory path.
...
The same loaded header snapshot is also applied to the dedicated `DetailFileStatController`, so the statistics tab reads
