...
## Responsibility
`DetailPanelController.cpp` owns the active detail-panel state, the active section view-model pointer, the toolbar
selection list, the dedicated `fileStat` statistics controller, and the three `.wsnhead`-backed selector-copy objects
used by the properties form.
That behavior is now the concrete note-detail implementation reused by `NoteDetailPanelController`; resource detail
...
- The `fileStat` page no longer resolves to a generic placeholder section object; it now exposes
  `DetailFileStatController` backed by the active header snapshot.
- The properties form selectors are now backed by dedicated `DetailHierarchySelectionController` objects, so hierarchy clicks in the sidebar do not rewrite detail-panel combo state.
- `main.cpp` injects the canonical Projects/Bookmarks/Progress hierarchy controllers only as option sources for these copies.
- The current note context now follows `SidebarHierarchyController` active bindings instead of being pinned to the library hierarchy, so the detail panel reads and writes the `.wsnhead` file for the note that the current workspace view actually identified.
...
- `reloadCurrentHeader(...)` now applies the loaded header snapshot to both `DetailPropertiesController` and
  `DetailFileStatController`, and clears both surfaces together when the note context is invalid.
- The controller now exports `noteContextLinked`; `reloadCurrentHeader(...)` sets it to `true` only when the active note
  id/path resolve and the `.wsnhead` snapshot is loadable, otherwise it clears both content controllers and marks the
  panel as detached.
...
- The current note header's progress enums are now only a fallback option source when no canonical
  Progress hierarchy controller is injected.
...
- Folder/tag add popups do not mutate the hierarchy source controllers directly; they only read those hierarchy nodes as
  option sources and keep the actual note-header write ownership inside `DetailPanelController`.
