...
- RAW rewrite authority therefore now stays at the editor-flow layer:
  ...
  - convert the visible selection range into direct RAW boundary inserts with
    `ContentsRawInlineStyleMutationSupport.buildInlineStyleSelectionPayload(...)`
...
- It no longer allocates a formatter QObject for selection writes. Selection formatting now routes through the pure
  RAW helper under `src/app/models/editor/format/ContentsRawInlineStyleMutationSupport.js`.
- Selected-range formatting no longer rebuilds a canonical style-coverage projection for the whole block. It wraps the
  resolved RAW selection span directly and leaves unrelated surrounding tags untouched.
