...
- Delegates Enter inside rendered `<style>` text to `NoteEditorDocumentSession.handleStyleBoundaryKeyInSource(...)`,
  so the key exits the style wrapper and moves the caret to the following source line before native `TextEdit` can
  flatten the styled rich text.
...
- callout boundary와 style Enter boundary는 `NoteEditorDocumentSession`으로 위임하고, 이미지 resource paste는
  `ClipboardEditorPaste`로 위임한다.
