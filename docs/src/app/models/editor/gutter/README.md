...
- Sample mounted editor geometry through `editorGeometryHost.lineStartRectangle(...)` whenever possible so wrapped text,
  rendered resources, and live text layout decide each source line's y position.
...
- 기준: QML은 LVRS 토큰, RAW source text, 표시용 평탄 블록, 논리 document block, parser/resource metadata, mounted editor geometry host와 gutter map target을 넘기고, 계산과 폴백은 `ContentsGutterLayoutMetrics`, `ContentsGutterLineNumberGeometry`, `ContentsGutterMarkerGeometry`가 담당한다. 라인 번호 개수는 RAW source의 물리 line start 기준이며, parser/display block은 해당 source line을 annotate할 뿐 row count를 늘리지 않는다. y는 가능한 경우 실제 에디터 geometry를 gutter 좌표계로 변환해 사용하고, 남는 content height를 특정 줄에 임의 배정하지 않는다.
