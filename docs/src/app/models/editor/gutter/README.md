...
- Own line-number y-position projection from the live `LV.TextEditor` geometry through
...
- 기준: QML은 LVRS 토큰과 에디터 geometry 훅을 입력값으로 넘기고, 계산과 폴백은 `ContentsGutterLayoutMetrics`, `ContentsGutterLineNumberGeometry`, `ContentsGutterMarkerGeometry`가 담당한다.
