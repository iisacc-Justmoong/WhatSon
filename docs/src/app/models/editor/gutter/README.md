...
- Measure RichText-visible lines through the inline editor's logical plain-text display probe, and keep consecutive
  line-number y values strictly increasing so line 1/2 cannot overlap during layout refresh.
...
- `contentsGutterLineNumberGeometry_separatesDuplicateDisplaySamples`
...
- 기준: QML은 LVRS 토큰과 현재 보이는 에디터 표시 geometry 훅을 입력값으로 넘기고, 계산과 폴백은 `ContentsGutterLayoutMetrics`, `ContentsGutterLineNumberGeometry`, `ContentsGutterMarkerGeometry`가 담당한다. RichText HTML 태그 위치가 아니라 보이는 논리 텍스트 probe를 측정하며, 중복 y 좌표는 겹치지 않게 보정한다.
