...
- Resource blocks remain one logical row and one gutter-line height. The logical text bridge exposes each resource as
  ...
  and applies only the explicit resource image-height delta to following rows, so text after an image frame lands below
  the frame without asking the RichText overlay or rendered HTML string to map unrelated logical offsets. That delta is
  anchored to the
...
얻은 자기 y snapshot을 사용한다. rendered mode에서는 명시적 resource image-height delta만 이후 row에 더하고,
