...
- Exposes `parsedLineCount` as canonical RAW source line metadata. The value is derived before source is projected into
  editor HTML or after synced editor HTML is converted back to source; the visual gutter row count comes from the LVRS
  editor surface's rendered line count in QML.
...
- `parsedLineCount`는 canonical RAW source line metadata이며 QML이 직접 파일을 읽거나 파싱하지 않게 한다.
  거터의 실제 row 개수는 LVRS editor surface의 rendered line count를 따른다.
