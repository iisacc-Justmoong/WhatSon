...
- `ContentsEditorGeometryProvider` is the QML-visible adapter that receives view-owned TextEdit/resource/target items
  and translates their `positionToRectangle(...)`, mapping, explicit resource visual heights, and visual-line probes
  into measured value snapshots. `renderedHtml` remains a compatibility fallback only and must not be the normal QML
  geometry input.
- Line-number row placement measures ordinary rows from the plain logical display item. The rendered resource item is
  not queried for following text-row offsets. Atomic resource frame height comes from `resourceVisualHeights`, which is
  computed by the inline resource presentation controller from resolved resource entries before HTML is consumed by the
  geometry adapter. The gutter metrics consumer uses that delta for placement only and keeps the resource allocation to
...
- `contentsEditorGeometryProvider_usesExplicitResourceVisualHeights`
...
- 기준: 거터/줄 번호/미니맵 계산 객체는 측정 snapshot만 사용하고 TextEdit, cursor, selection, resource overlay
  ...
  frame의 vertical delta는 rendered HTML 문자열 파싱이 아니라 `resourceVisualHeights` 입력을 우선 사용한다.
