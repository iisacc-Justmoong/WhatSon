...
- `lineNumber/ContentsLineNumberRailMetrics.*` owns logical line-number rail row construction. QML passes source,
  renderer metadata, and measured row-geometry snapshots; C++ derives rows from authoritative `sourceText` through its
  internal logical bridge, maps those logical lines back to RAW source offsets, and publishes row y/height values
  without touching cursor, selection, TextEdit, or overlay objects.
