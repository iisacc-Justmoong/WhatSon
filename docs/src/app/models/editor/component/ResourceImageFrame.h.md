...
- `ResourceFrameDescriptor` is the normalized data package consumed by the renderer. It contains the canonical
  ...
  the current editor viewport width used for the single image object's intrinsic render width.
...
- standalone `<resource ... />` 라인의 HTML frame, editor viewport 폭을 intrinsic width로 쓰는 responsive frame,
  Figma 480px design baseline 기준의 고정 chrome metric, 이미지 source 비율 기반 auto-height와 1:1 최대 display
  box, 입력란이 아닌 type/file name 표시란, synthetic outer block 없는 단일 image object 렌더링, 이전 table chrome
  residue 저장 복원 시 건너뛸 렌더 텍스트 목록은 `ResourceFrame`이 소유한다.
