...
- `ResourceFrame` owns the display contract for standalone editor image resource blocks:
  ...
  - editor-width media raster rendering with auto height, dynamic centered image placement, and a square 1:1 maximum
    display box
...
- standalone `<resource ... />` 라인의 HTML frame, editor viewport 폭을 intrinsic width로 쓰는 responsive image
  ...
  이미지 source 비율 기반 auto-height와 1:1 최대 display box, synthetic outer block 없는 image-only frame
  container 렌더링을 `ResourceFrame`이 소유한다. type, `...`,
