...
- The frame image itself carries zero top/bottom/left/right margins and uses `vertical-align:top`. Source-line placement
  is owned by the editor session projection, so the rendered object must not add an extra spacer row above the resource
  line.
- There are no visible header/footer text surfaces in the current image-only frame contract.

## Runtime Behavior

- Image resources render through an editor-width cached media raster with explicit `width`/`height` attributes, while the
  decoded image display box is centered inside that frame-width raster.
...
- 이 구현은 Figma `292:50`의 이미지 resource frame 구조를 Qt rich text HTML로 변환한다. 실제 HTML에는
  `whatson-resource-frame whatson-resource-media` 단일 `<img>`를 노출하고, 캐시 raster 자체가 frame container
  역할을 한다.
...
- 이미지 영역은 현재 에디터 폭과 계산된 frame 높이를 명시적 `width`/`height` 속성으로 내보낸다. 높이는 이미지 source 해상도의 비율을
