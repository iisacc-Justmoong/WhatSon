...
- Consumes plain Backspace/Enter only when `NoteEditorDocumentSession.handleCalloutBoundaryKeyInSource(...)` reports a
  handled callout boundary operation.
...
- 콜아웃 경계 키와 이미지 리소스 paste shortcut의 event consume 여부는 이 객체가 결정한다.
- 실제 clipboard resource import는 `ClipboardEditorPaste`에, callout RAW 판단과 source mutation은
