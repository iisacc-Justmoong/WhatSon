...
- Keeps the `LV.TextEditor` read-only so this Figma artifact cannot become a write-authoritative note-body editor.
- Does not install custom key handlers or input-method calls.

## UI
- Uses `LV.TextEditor` for text projection and selection.
