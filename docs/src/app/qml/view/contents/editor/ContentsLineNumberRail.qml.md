...
- Each row represents one logical document line, not one wrapped visual text row.
...
`ContentsInlineFormatEditor.qml` measures the live editor geometry and publishes row objects with `number`, `y`, and
`height`. `ContentsDisplayView.qml` mounts this line-number rail inside the same `Flickable` content item as
