...
- Keeps a disabled, transparent plain `TextEdit` geometry probe in sync with the logical display text for cursor hit
  testing, projected caret placement, and gutter line-start lookup.
- Exposes line-start geometry from that logical plain-text probe, then adds the cumulative y delta introduced by any
  rendered iiHtmlBlock resource frames before the requested source line. This avoids using RichText character offsets
  as document-line coordinates after images and HTML paragraphs have changed the overlay's internal text positions.
- When the requested source position belongs to an iiHtmlBlock resource span, `positionToRectangle(...)` returns the
  rendered resource frame rectangle from the RichText overlay rather than the one-line plain-text probe rectangle. This
  lets the gutter assign a tall row to a resource line even when it is the last numbered row.
...
- When the document has no atomic iiHtmlBlock resource spans, native composition can still use the plain
  `LV.TextEditor.editorItem` fallback geometry. When iiHtmlBlock resource spans are present, the rendered overlay is
  pinned even during composition so ordinary typing cannot expose the RAW `<resource ... />` tag while the renderer
  catches up.
...
projected caret follows the visible text rather than hidden markup; surface clicks perform the inverse logical-to-RAW
offset conversion before moving the underlying editor cursor. The native cursor delegate is hidden in that state and
re-enabled only when the rendered overlay is unavailable.
