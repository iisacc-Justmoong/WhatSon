...
- `ContentsInlineFormatEditor.qml` keeps editing on an `LV.TextEditor` plain-text buffer while displaying the read-side
  ...
  native RAW cursor delegate are hidden. The rendered surface does not install pointer handlers above the editor;
  clicks, drags, and modifier-based selection stay on the native `LV.TextEditor` path. Inline resource frames are
  rendered at 100% of the editor text-column width. The editor body has no top inset or overlay padding, so the first
  text line starts at the top of the document slot.
...
- pointer: 렌더 표면은 별도 포인터 핸들러를 두지 않고 `LV.TextEditor`의 OS/Qt 선택 경로를 그대로 사용한다.
