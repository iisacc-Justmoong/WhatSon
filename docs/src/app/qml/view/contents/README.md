...
- `ContentsDisplayView.qml` imports the parent `view/contents` directory as `ContentsChrome` and reuses
  `Gutter.qml` and `Minimap.qml` from this namespace.

## Tests

- `test/cpp/suites/qml_contents_view_tests.cpp` locks the four-file standalone composition contract and the read-only
...
- 배치: `src/app/qml/contents`와 `src/app/qml/view/content`는 재도입하지 않고, `Gutter.qml`, `EditorView.qml`, `Minimap.qml`, `ContentsView.qml`, 그리고 `editor/*`를 모두 `src/app/qml/view/contents` 아래에 둔다.
