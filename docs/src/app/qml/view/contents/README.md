...
- Minimap visible-line measurement is delegated to a host-owned `ContentsEditorVisualLineMetrics`, and minimap
  arithmetic is delegated to `ContentsMinimapLayoutMetrics`; the QML files render resolved values. Runtime editor hosts
  must feed those models from an independent metric probe layer and primitive LVRS token values, then feed
  `Minimap.qml` with matching per-row width ratios and scroll-drag enablement. `Minimap.qml` renders rows inside an 8px
  left/right inset, does not render a viewport thumb or scrollbar chrome, and drag movement is emitted as a direct
  pixel delta instead of a normalized scrollbar ratio.
...
- 기준: 파일 경로, 명령, API 이름, 세부 변경 이력은 위 영어 본문을 원문 기준으로 유지한다. 런타임 미니맵 행 수와 행별 폭은 host가 소유한 독립 metric probe의 wrap 결과 줄과 표시 길이를 따르며, 행 폭은 좌우 8px inset을 제외한 내부 rail 기준으로 계산한다. 미니맵 세로 드래그는 host viewport에 같은 pixel delta로 연결하되 별도 viewport thumb이나 스크롤바 chrome은 표시하지 않는다.
