...
- Standalone `ContentsView.qml` and the runtime `ContentsDisplayView.qml` both apply `LV.Theme.gap8` as a shared top
  and bottom chrome inset, so editor, gutter, and minimap surfaces start and end inside the same vertical padding.
...
- padding: standalone `ContentsView.qml`와 runtime `ContentsDisplayView.qml`는 모두 상하 `LV.Theme.gap8` inset을
