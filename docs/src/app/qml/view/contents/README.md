...
- Minimap arithmetic is delegated to `ContentsMinimapLayoutMetrics`; the QML files render resolved values. Runtime
  editor hosts must feed that model from the visible editor surface's wrapped line count and feed `Minimap.qml` with
  matching per-row width ratios and viewport scroll ratios.
- Runtime editor gutters are view-only QML chrome. They count logical lines from the structured editor projection while
  measuring each row's y/height from the live editor geometry, and they must remain inside the body viewport rather
  than becoming detached overlays.
...
- gutter: 런타임 거터는 논리 줄 수를 번호로 세고, 각 번호 행의 y/height는 실제 에디터 지오메트리에서
  측정한다. 본문 viewport 안에 함께 배치되어 스크롤 좌표계를 공유해야 한다.
