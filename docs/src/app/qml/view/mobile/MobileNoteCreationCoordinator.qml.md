...
- Resolve the writable note-mutation capability through `windowInteractions.resolveLibraryNoteCreationController()`.
- Listen for `emptyNoteCreated(noteId)` from that note-mutation controller.
...
## Key Functions
- `syncNoteCreationController()`: refreshes the writable note-mutation reference from the interaction controller.
...
- `routePendingCreatedNoteToEditor()`: emits `openEditorRequested(...)` only when the routed mobile page has the
  required content controller, note-list model, and page router in place.
...
- `MobileHierarchyPage.qml`: consumes `openEditorRequested(...)` and bridges the coordinator into the routed mobile
  page stack.
- `LibraryNoteMutationController`: emits `emptyNoteCreated(noteId)` after file creation succeeds.
