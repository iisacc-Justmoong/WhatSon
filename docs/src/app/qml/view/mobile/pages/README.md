...
- The file keeps the routed hierarchy, note-list, editor, and detail shell. The editor route uses
  `ContentViewLayout.qml`, forwards `noteActiveState`, and does not forward editor view-mode controller state.
...
- editor route는 `ContentViewLayout.qml`을 사용하고 active note body file을 `LV.TextEditor`에 연결한다.
