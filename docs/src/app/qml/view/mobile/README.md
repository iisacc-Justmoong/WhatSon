...
- `MobileEventSurface.qml` has been removed. The mobile editor route relies on the native `LV.TextEditor` path and must
  not add an editor-level touch, pointer, gesture, or LVRS event-listener overlay.
...
- `MobileEventSurface.qml`은 삭제되었으며 editor route는 별도 touch, pointer, gesture, LVRS event listener overlay를
  추가하지 않는다.
