...
- Header:
  - header height resolves through `LV.Theme.headerMinHeight - LV.Theme.gap2` (`monthHeaderHeight`)
...
- Header/body typography and weekday/header paddings now route through named `LV.Theme` text, header, and gap tokens
  instead of raw `12/22/39/54px` literals.
- Transparent surfaces now use `LV.Theme.accentTransparent`.
