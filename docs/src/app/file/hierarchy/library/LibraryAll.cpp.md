...
- Runtime body indexing now also carries a dedicated editor-source projection (`bodySourceText`) built from `.wsnbody` rich text parsing, while keeping `bodyPlainText` for search/preview filtering.
