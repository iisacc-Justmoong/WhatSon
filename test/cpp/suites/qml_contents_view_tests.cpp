...
void WhatSonCppRegressionTests::qmlContentsView_keepsOnlyGutterTextEditorMinimapViews()
{
    ...
    QVERIFY(textEditorSource.contains(QStringLiteral("readonly property int editorCursorLineIndex: textEditor.cursorLineIndexForVisualCursor()")));
    ...
    QVERIFY(textEditorSource.contains(QStringLiteral("function cursorLineIndexForVisualCursor()")));
    QVERIFY(textEditorSource.contains(QStringLiteral("slice(0, safeCursorPosition).split(\"\\n\").length - 1")));
    QVERIFY(textEditorSource.contains(QStringLiteral("editorSurface.cursorRectangle")));
    QVERIFY(textEditorSource.contains(QStringLiteral("const cursorY = Math.max(")));
    QVERIFY(textEditorSource.contains(QStringLiteral("cursorY >= top && cursorY < bottom")));
