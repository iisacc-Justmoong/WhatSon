...
void WhatSonCppRegressionTests::contentsDisplayMinimapCoordinator_buildsStructuredMinimapGroups()
{
...
void WhatSonCppRegressionTests::minimapSnapshotSupport_preservesVisualRowWidths()
{
    ...
    const QString minimapSnapshotSupportSource = readUtf8SourceFile(
        QStringLiteral("src/app/models/editor/display/ContentsMinimapSnapshotSupport.js"));
    QVERIFY(!minimapSnapshotSupportSource.isEmpty());
    QVERIFY(!minimapSnapshotSupportSource.contains(QStringLiteral("function computeChangedLineRange")));
    QVERIFY(!minimapSnapshotSupportSource.contains(QStringLiteral("function spliceLineGroups")));
